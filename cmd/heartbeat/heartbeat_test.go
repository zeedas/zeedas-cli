...
import (
	...
	"github.com/zeedas/zeedas-cli/cmd"
...
func TestSendHeartbeats_ObfuscateProject(t *testing.T) {
	...
	router.HandleFunc("/users/current/heartbeats.bulk", func(w http.ResponseWriter, req *http.Request) {
		...
		lines, err := project.ReadFile(filepath.Join(fp, "zeedas-cli", ".zeedas-project"), 1)
		require.NoError(t, err)
	...
	v.Set("category", "debugging")
	v.Set("cursorpos", 42)
	v.Set("entity", filepath.Join(fp, "zeedas-cli/src/pkg/file.go"))
	v.Set("entity-type", "file")
	v.Set("key", "00000000-0000-4000-8000-000000000000")
	v.Set("language", "Go")
	v.Set("alternate-language", "Golang")
