...
import (
	...
	"github.com/zeedas/zeedas-cli/pkg/api"
...
// defaultConfigSection is the default section in the zeedas ini config file.
...
func setFlags(cmd *cobra.Command, v *viper.Viper) {
	...
	flags.String(
		"api-url",
		"",
		"API base url used when sending heartbeats and fetching code stats. Defaults to https://api.zeedas.com/api/v1/.",
	)
	flags.String(
		"apiurl",
		"",
		"(deprecated) API base url used when sending heartbeats and fetching code stats. Defaults to"+
			" https://api.zeedas.com/api/v1/.",
	)
	flags.String(
		"category",
		"",
	...
	flags.String("internal-config", "", "Optional internal config file. Defaults to '~/.zeedas/zeedas-internal.cfg'.")
	...
	flags.String(
		...
		"Obfuscate project names. When a project folder is detected instead of"+
			" using the folder name as the project, a .zeedas-project file is"+
	...
	flags.Bool(
		...
		false,
		"Disables tracking folders unless they contain a .zeedas-project file. Defaults to false.",
	)
	flags.Bool(
		"is-unsaved-entity",
		false,
	...
	flags.String("key", "", "Your zeedas api key; uses api_key from ~/.zeedas.cfg by default.")
	...
	flags.String("log-file", "", "Optional log file. Defaults to '~/.zeedas/zeedas.log'.")
	flags.String("logfile", "", "(deprecated) Optional log file. Defaults to '~/.zeedas/zeedas.log'.")
	...
	flags.Int(
		...
		fmt.Sprintf("Amount of offline activity to sync from your local ~/.zeedas.bdb bolt"+
			" file to your zeedas Dashboard before exiting. Can be zero or"+
			" a positive integer. Defaults to %d, meaning after sending a heartbeat"+
			" while online, all queued offline heartbeats are sent to zeedas API, up"+
	...
	flags.String(
		...
		"Prints time for the given goal id today, then exits"+
			" Visit zeedas.com/api/v1/users/current/goals to find your goal id.")
	flags.Bool(
		"user-agent",
		false,
		"(internal) Prints the zeedas-cli useragent, as it will be sent to the api, then exits.",
	)
	flags.Bool("verbose", false, "Turns on debug messages in log file, and sends diagnostics if a crash occurs.")
	flags.Bool("version", false, "Prints the zeedas-cli version number, then exits.")
...
func Execute() {
	if err := NewRootCMD().Execute(); err != nil {
		log.Fatalf("failed to run zeedas-cli: %s", err)
	}
}
