...
import (
	...
	paramscmd "github.com/zeedas/zeedas-cli/cmd/params"
	"github.com/zeedas/zeedas-cli/pkg/api"
	"github.com/zeedas/zeedas-cli/pkg/apikey"
	"github.com/zeedas/zeedas-cli/pkg/heartbeat"
	inipkg "github.com/zeedas/zeedas-cli/pkg/ini"
	"github.com/zeedas/zeedas-cli/pkg/output"
...
func TestLoadParams_Filter_Exclude(t *testing.T) {
	v := viper.New()
	v.Set("entity", "/path/to/file")
	v.Set("exclude", []string{".*", "zeedas.*"})
	v.Set("settings.exclude", []string{".+", "zeedas.+"})
	v.Set("settings.ignore", []string{".?", "zeedas.?"})
	...
	assert.Equal(t, "(?i)zeedas.*", params.Filter.Exclude[1].String())
	assert.Equal(t, "(?i).+", params.Filter.Exclude[2].String())
	assert.Equal(t, "(?i)zeedas.+", params.Filter.Exclude[3].String())
	assert.Equal(t, "(?i).?", params.Filter.Exclude[4].String())
	assert.Equal(t, "(?i)zeedas.?", params.Filter.Exclude[5].String())
}
...
func TestLoadParams_Filter_Include(t *testing.T) {
	v := viper.New()
	v.Set("entity", "/path/to/file")
	v.Set("include", []string{".*", "zeedas.*"})
	v.Set("settings.include", []string{".+", "zeedas.+"})
	...
	assert.Equal(t, "(?i)zeedas.*", params.Filter.Include[1].String())
	assert.Equal(t, "(?i).+", params.Filter.Include[2].String())
	assert.Equal(t, "(?i)zeedas.+", params.Filter.Include[3].String())
}
	...
	tests := map[string]struct {
		...
		"api key flag takes precedence": {
			...
			Expected: paramscmd.API{
				...
				URL:      "https://api.zeedas.com/api/v1",
...
func TestLoadParams_ApiKey_FromVault(t *testing.T) {
	v := viper.New()
	v.Set("config", "testdata/.zeedas-vault.cfg")
	v.Set("entity", "testdata/heartbeat_go.json")
...
func TestLoadParams_ApiKey_FromVault_Err_Darwin(t *testing.T) {
	...
	v := viper.New()
	v.Set("config", "testdata/.zeedas-vault-error.cfg")
	v.Set("entity", "testdata/heartbeat_go.json")
...
func TestLoad_API_ProxyURL_NoProxyFromEnvironment(t *testing.T) {
	...
	err := os.Setenv("NO_PROXY", "https://some.org,https://api.zeedas.com")
	require.NoError(t, err)
