...
import (
	...
	"github.com/zeedas/zeedas-cli/pkg/system"
...
// Result represents a response from the zeedas api.
type Result struct {
	Errors    []string
	Status    int
	Heartbeat Heartbeat
...
// Sender sends heartbeats to the zeedas api.
...
// PluginFromUserAgent parses the plugin name from a zeedas user agent.
