...
func TestReadInConfig_Multiline(t *testing.T) {
	...
	v.Set("config", "testdata/zeedas-multiline.cfg")
...
func TestReadInConfig_Multiple(t *testing.T) {
	...
	v.Set("config", "testdata/zeedas.cfg")
	v.Set("internal-config", "testdata/zeedas-internal.cfg")
...
func TestInternalFilePath(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"default": {
			Expected: filepath.Join(home, ".zeedas", "zeedas-internal.cfg"),
		},
		"env trailing slash": {
			EnvVar:   "~/path2/",
			Expected: filepath.Join(home, "path2", "zeedas-internal.cfg"),
		},
		"env without trailing slash": {
			EnvVar:   "~/path2",
			Expected: filepath.Join(home, "path2", "zeedas-internal.cfg"),
...
func TestWrite(t *testing.T) {
	tmpFile, err := os.CreateTemp(t.TempDir(), "zeedas")
	require.NoError(t, err)
...
func TestWrite_NoMultilineSideEffects(t *testing.T) {
	...
	copyFile(t, "testdata/zeedas-multiline.cfg", tmpFile.Name())
	...
	expected, err := os.ReadFile("testdata/zeedas-multiline-expected.cfg")
	require.NoError(t, err)
...
func TestWrite_NullsRemoved(t *testing.T) {
	...
	copyFile(t, "testdata/zeedas-nulls.cfg", tmpFile.Name())
	...
	expected, err := os.ReadFile("testdata/zeedas-nulls-expected.cfg")
	require.NoError(t, err)
...
func TestWriteErr(t *testing.T) {
	...
	assert.Equal(t, "got undefined zeedas config file instance", err.Error())
}
