...
import (
	...
	"github.com/zeedas/zeedas-cli/pkg/vipertools"
...
// WakaHomeType is zeedas home type.
type WakaHomeType int

const (
	defaultFolder = ".zeedas"
	// WakaHomeTypeUnknown is unknown zeedas home type.
	WakaHomeTypeUnknown WakaHomeType = iota
	// WakaHomeTypeEnvVar is zeedas home type from environment variable.
	WakaHomeTypeEnvVar
	// WakaHomeTypeOSDir is zeedas home type from OS directory.
	WakaHomeTypeOSDir
)

const (
	// defaultFile is the name of the default zeedas config file.
	defaultFile = ".zeedas.cfg"
	// defaultInternalFile is the name of the default zeedas internal config file.
...
func (w *WriterConfig) Write(section string, keyValue map[string]string) error {
	if w.File == nil || w.ConfigFilepath == "" {
		return errors.New("got undefined zeedas config file instance")
	}
	...
	if err := w.File.SaveTo(w.ConfigFilepath); err != nil {
		return fmt.Errorf("error saving zeedas config: %s", err)
	}
...
// ImportFilePath returns the path for import zeedas config file.
...
// InternalFilePath returns the path for the zeedas internal config file.
...
func WakaHomeDir() (string, WakaHomeType, error) {
	...
	return "", WakaHomeTypeUnknown, fmt.Errorf("could not determine zeedas home dir")
}

// WakaResourcesDir returns the ~/.zeedas/ folder.
