...
// zeedas project file can be detected in the filepath directory tree.
...
func filterFileEntity(h heartbeat.Heartbeat, config Config) error {
	...
	// when including only with project file, skip files when the project doesn't have a .zeedas-project file
	if config.IncludeOnlyWithProjectFile {
		_, ok := project.FindFileOrDirectory(entity, project.ZeedasProjectFile)
		if !ok {
			return fmt.Errorf("skipping because missing .zeedas-project file in parent path")
		}
	}
