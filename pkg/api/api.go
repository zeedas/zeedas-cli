...
const (
	// BaseURL is the base url of the zeedas api.
	BaseURL = "https://api.zeedas.com/api/v1"
	// baseIPAddrv4 is the base ip address v4 of the zeedas api.
	baseIPAddrv4 = "143.244.210.202"
	// baseIPAddrv6 is the base ip address v6 of the zeedas api.
	baseIPAddrv6 = "2604:a880:4:1d0::2a7:b000"
	// DefaultTimeoutSecs is the default timeout used for requests to the zeedas api.
	DefaultTimeoutSecs = 120
)

// Client communicates with the zeedas api.
type Client struct {
	baseURL string
	client  *http.Client
...
func isLocalIPv6() bool {
	...
	defer func() {
		if err := conn.Close(); err != nil {
			log.Debugf("failed to close connection to api zeedas: %s", err)
		}
	}()
