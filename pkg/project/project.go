...
const (
	// zeedasProjectFile is the special file which if present should contain the project name and optional branch name
	// that will be used instead of the auto-detected project and branch names.
	ZeedasProjectFile = ".zeedas-project"
	zeedasProjectFile = ".zeedas-project"
...
// First looks for a .zeedas-project file or project map. Second, uses the
	...
	return func(next heartbeat.Handle) heartbeat.Handle {
		return func(hh []heartbeat.Heartbeat) ([]heartbeat.Result, error) {
			for n, h := range hh {
				...
				// first, use .zeedas-project or [projectmap] section with entity path.
...
// Write saves zeedas project file.
func Write(folder, project string) error {
	err := os.WriteFile(filepath.Join(folder, ZeedasProjectFile), []byte(project+"\n"), 0600)
	if err != nil {
		return fmt.Errorf("failed to save zeedas project file: %s", err)
	}
