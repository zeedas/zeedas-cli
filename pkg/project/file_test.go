...
func TestFile_Detect_FileExists(t *testing.T) {
	...
	f := project.File{
		Filepath: filepath.Join(tmpDir, ".zeedas-project"),
	}
...
func TestFile_Detect_NoFileFound(t *testing.T) {
	...
	tmpFile, err := os.CreateTemp(tmpDir, "zeedas-project")
	require.NoError(t, err)
...
func TestFindFileOrDirectory(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"filename": {
			Filepath: dir,
			Filename: ".zeedas-project",
			Expected: filepath.Join(tmpDir, ".zeedas-project"),
		},
		"directory": {
			Filepath: dir,
			Filename: "src",
