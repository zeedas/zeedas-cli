...
// Detect get information from a .zeedas-project file about the project for
// a given file. First line of .zeedas-project sets the project
// name. Second line sets the current branch name.
func (f File) Detect() (Result, bool, error) {
	fp, ok := FindFileOrDirectory(f.Filepath, zeedasProjectFile)
	if !ok {
		return Result{}, false, nil
	}

	log.Debugf("zeedas project file found at: %s", fp)
