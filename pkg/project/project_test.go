...
func TestWithDetection_zeedasProjectTakesPrecedence(t *testing.T) {
	fp := setupTestGitBasic(t)

	entity := filepath.Join(fp, "zeedas-cli/src/pkg/file.go")
	projectPath := filepath.Join(fp, "zeedas-cli")
	...
	copyFile(
		t,
		"testdata/zeedas-project-other",
		filepath.Join(fp, "zeedas-cli", ".zeedas-project"),
	)
...
func TestWithDetection_ObfuscateProject(t *testing.T) {
	...
	assert.FileExists(t, filepath.Join(fp, "zeedas-cli/.zeedas-project"))
}
...
func TestDetect_FileDetected(t *testing.T) {
	...
	copyFile(
		t,
		"testdata/zeedas-project",
		filepath.Join(tmpDir, ".zeedas-project"),
	)
	...
	assert.Equal(t, "zeedas-cli", result.Project)
...
func TestWrite(t *testing.T) {
	...
	actual, err := os.ReadFile(filepath.Join(tmpDir, ".zeedas-project"))
	require.NoError(t, err)
