...
func TestQueueFilepath(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"default": {
			Expected: filepath.Join(home, ".zeedas.bdb"),
		},
		"env_trailling_slash": {
			EnvVar:   "~/path2/",
			Expected: filepath.Join(home, "path2", ".zeedas.bdb"),
		},
...
func TestWithQueue(t *testing.T) {
	...
	assert.Equal(t, "1592868394.084354-file-building-zeedas-todaygoal-/tmp/main.js-false", stored[0].ID)
...
func TestWithQueue_ApiError(t *testing.T) {
	...
	assert.Equal(t, "1592868367.219124-file-coding-zeedas-cli-heartbeat-/tmp/main.go-true", stored[0].ID)
	assert.JSONEq(t, string(dataGo), stored[0].Heartbeat)

	assert.Equal(t, "1592868386.079084-file-debugging-zeedas-summary-/tmp/main.py-false", stored[1].ID)
...
func TestWithQueue_InvalidResults(t *testing.T) {
	...
	assert.Equal(t, "1592868386.079084-file-debugging-zeedas-summary-/tmp/main.py-false", stored[0].ID)
	assert.JSONEq(t, string(dataPy), stored[0].Heartbeat)

	assert.Equal(t, "1592868394.084354-file-building-zeedas-todaygoal-/tmp/main.js-false", stored[1].ID)
...
func TestSync_MultipleRequests(t *testing.T) {
	...
	for i := 0; i < 26; i++ {
		insertHeartbeatRecord(t, db, "heartbeats", heartbeatRecord{
			ID:        strconv.Itoa(i) + "1592868367.219124-file-coding-zeedas-cli-heartbeat-/tmp/main.go-true",
			Heartbeat: string(dataGo),
		})
	}
...
func TestQueue_PushMany(t *testing.T) {
	...
	assert.Equal(t, "1592868394.084354-file-building-zeedas-todaygoal-/tmp/main.js-false", stored[2].ID)
	...
	return []heartbeat.Heartbeat{
		{
			...
			Project:        heartbeat.PointerTo("zeedas-cli"),
			Time:           1592868367.219124,
			UserAgent:      "zeedas/13.0.6",
		},
		{
			...
			Project:        heartbeat.PointerTo("zeedas"),
			Time:           1592868386.079084,
			UserAgent:      "zeedas/13.0.7",
		},
		{
