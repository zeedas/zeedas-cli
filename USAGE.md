...
Options can be passed to zeedas-cli via command line, or set in the `$zeedas_HOME/.zeedas.cfg` config file.
`$zeedas_HOME` defaults to your user's `$HOME` directory.
Command line arguments take precedence over config file settings.
Run `zeedas-cli --help` for available command line options.

## INI Config File

Here's an example `$zeedas_HOME/.zeedas.cfg` config file with all available options:
...
api_url = https://api.zeedas.com/api/v1
...
| api_key                        | Your zeedas api key. | _string_ | |
| api_key_vault_cmd              | Any shell command to get your api key from vault. | _string_ | |
| api_url                        | The zeedas API base url. | _string_ | <https://api.zeedas.com/api/v1> |
| hide_file_names                | Obfuscate filenames. Will not send file names to api. | _bool_;_list_ | `false` |
| hide_project_names             | Obfuscate project names. When a project folder is detected instead of using the folder name as the project, a `.zeedas-project file` is created with a random project name. | _bool_;_list_ | `false` |
...
| include_only_with_project_file | Disables tracking folders unless they contain a `.zeedas-project file`. | _bool_ | `false` |
| exclude_unknown_project        | When set, any activity where the project cannot be detected will be ignored. | _bool_ | `false` |
| status_bar_enabled             | Turns on zeedas status bar for certain editors. | _bool_ | `true` |
| status_bar_coding_activity     | Enables displaying Today's code stats in the status bar of some editors. When false, only the zeedas icon is displayed in the status bar. | _bool_ | `true` |
...
| offline                        | Enables saving code stats locally to ~/.zeedas.bdb when offline, and syncing to the dashboard later when back online. | _bool_ | `true` |
...
| log_file                       | Optional log file path. | _filepath_ | `~/.zeedas/zeedas.log` |
| import_cfg                     | Optional path to another zeedas.cfg file to import. If set it will overwrite values loaded from $zeedas_HOME/.zeedas.cfg file. | _filepath_ | |
...
For commonly used configuration options, see examples in the [FAQ](https://zeedas.com/faq).

## Internal INI Config File

The plugins and waktime-cli use a separate internal INI file for things like caching auto-update requests to the GitHub releases API, and exponential backoff to the zeedas API.
The default internal INI config file location is `$zeedas_HOME/.zeedas/zeedas-internal.cfg`.
